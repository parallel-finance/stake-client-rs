...
use substrate_subxt::{balances, Client, ClientBuilder, PairSigner, Signer, staking};
...
pub async fn test_multisig_balances_transfer(
    ...
    let call =
        kusama::api::balances_transfer_call::<RelayRuntime>(&dest, 3_000_000_000_000u128);
    ...
    let kusama::api::MultisigData { when, .. } =
        subxt_client.fetch(&store, None).await?.unwrap();
...
pub async fn test_multisig_staking_bond(
    ...
    let dest = AccountKeyring::Eve.to_account_id().into();
    let call =
        kusama::api::staking_bond_call::<RelayRuntime>(&dest, 15_000_000_000_000u128,staking::RewardDestination::Staked);
    let mc = kusama::api::multisig_approve_as_multi_call::<
        RelayRuntime,
